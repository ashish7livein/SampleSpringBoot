...
destAbsPath=/Users/ashishkumar.ashok/Desktop/$projectName/$env
...
#config files folder
sourConfigFolder=$WORKSPACE/src/main/$configFolder*
...
function stopServer(){
    ...
    pidToStop=`lsof -i tcp:$serverPort | awk '{print $2}' | sed '2q;d'`
    if [[ -n $pidToStop ]]
    then
    kill -9 $pidToStop
    echo "Congrates!! $serverPort is stopped."
    else
